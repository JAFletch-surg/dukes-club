...
import { GraduationCap, Plus, Edit, Trash2, Save, Loader, X } from 'lucide-react'
...
export default function FacultyAdmin() {
  ...
  const emptyForm = { full_name: '', position_title: '', hospital: '', bio: '', email: '', sort_order: 0, is_active: true }
  const [form, setForm] = useState(emptyForm)

  const openNew = () => { setForm(emptyForm); setEditing('new') }
  const openEdit = (f: any) => {
    setForm({ full_name: f.full_name || '', position_title: f.position_title || '', hospital: f.hospital || '', bio: f.bio || '', email: f.email || '', sort_order: f.sort_order || 0, is_active: f.is_active ?? true })
    setEditing(f.id)
  }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      if (editing === 'new') { await create(form); showToast('Faculty added') }
      else { await update(editing!, form); showToast('Faculty updated') }
      setEditing(null)
          ...
          <table className="w-full text-sm">
            ...
            <tbody>{faculty.map((f: any) => (
              <tr key={f.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                <td className="px-4 py-3"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-emerald-700 text-white rounded-full flex items-center justify-center text-xs font-bold">{initials(f.full_name || '?')}</div><span className="font-medium">{f.full_name}</span></div></td>
